...
// Quiz Submissions
import { quizSubmissions, InsertQuizSubmission } from "../drizzle/schema";

export async function createQuizSubmission(data: InsertQuizSubmission) {
  const db = await getDb();
  if (!db) {
  ...
  try {
    ...
    console.error("[Database] Failed to create quiz submission:", error);
    throw error;
  }
}

export async function getQuizSubmissions(limit: number = 100, offset: number = 0) {
  const db = await getDb();
  if (!db) {
